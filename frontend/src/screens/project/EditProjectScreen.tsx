...
export const EditProjectScreen: React.FC<EditProjectScreenProps> = ({ onNavigate, projectId }) => {
  const { user } = useAuth();
  ...
  const loadProject = async () => {
    try {
      ...
      setFormData({
        ...
        vendorPayRate: data.vendorPayRate?.toString() || '',
        candidatePayRate: data.candidatePayRate?.toString() || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await projectService.update(projectId, submitData);
      
      if (Platform.OS === 'web') {
        alert('✓ Project updated successfully');
        onNavigate('ViewProject', { id: projectId });
      } else {
        Alert.alert('Success', 'Project updated successfully', [
          { text: 'OK', onPress: () => onNavigate('ViewProject', { id: projectId }) }
        ]);
      }
    } catch (error) {
      if (Platform.OS === 'web') {
    ...
    <Screen>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <TouchableOpacity style={styles.backButton} onPress={() => onNavigate('ViewProject', { id: projectId })}>
        ...
        <View style={styles.buttonContainer}>
          <Button
            title="Cancel"
            onPress={() => onNavigate('ViewProject', { id: projectId })}
            variant="secondary"
            style={styles.button}
          />
          <Button
